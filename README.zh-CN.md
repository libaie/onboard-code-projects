...
AI 写代码最怕的不是模型不够聪明，而是**上下文串了**：A 仓库的指令混进 B 仓库的代码，切错分支，或者写进了不该写的地方。`onboard-code-projects` 给每个仓库一个经过核验、可复用的**项目绑定任务**和一份 `codebase-memory` 索引，再加一个可选**中控**：按任务难度与风险把派发自动分配到 `economy`、`balanced` 或 `frontier` 模型等级，复用有证据支撑的成功经验，同时拒绝已知的确定性失败。
...
- 🎯 **自适应模型分层**——`economy` / `balanced` / `frontier` 三个等级只在派发时才解析为当前可用的具体模型
- 🧾 **哈希链中控状态**——每次状态变更都经过测试过的适配器并做读后核验；派发台账哈希成链、按月归档
